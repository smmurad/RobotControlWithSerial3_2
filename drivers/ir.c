...
int16_t IrAnalogToMM(uint16_t reading, IR_Sensor_t sensor){
	...
	result = (int16_t) ceil((IR_SENSOR_1_DIVIDER*pow(reading,IR_SENSOR_1_EXPONENT))); 
	}
	if (sensor == IR_SENSOR_2){
	result = (int16_t) ceil( (IR_SENSOR_2_DIVIDER*pow(reading,IR_SENSOR_2_EXPONENT))); 
	}
	if(sensor == IR_SENSOR_3){
	result = (int16_t) ceil( (IR_SENSOR_3_DIVIDER*pow(reading,IR_SENSOR_3_EXPONENT)));
	}
	if (sensor == IR_SENSOR_4){
	result = (int16_t) ceil( (IR_SENSOR_4_DIVIDER*pow(reading,IR_SENSOR_4_EXPONENT)));
	}
	return result; //MIN(result,IR_MAX_DETECT_DISTANCE_MM);
}

		...
		*/
		...
		if (gHandshook) { // gHandshook set to true in IMU calibration further down
			...
			if((dRobot == 0) | (fabs(accel.x) < 0.05)){ //if (dRobot ==0)
				...
				if(fabs(Z[3]-Z[2])>0.2*Z[3]){ ekf_setEncoderVar(1);}		// if gyro and encoders mismatch (stuck?) trust gyro
				else {ekf_setEncoderVar(0.03);}
			}
			...
			*/
			...
			if(LOG_MEASUREMENT_DATA){
				...
				printf("%f %f %f %ld %ld %f %f %f\n\r",delta_t,accel.x,gyrZ,encoder_ticks.left, encoder_ticks.right,ekf_state.heading,ekf_state.x,ekf_state.y);
			}
/*			printf("x_hat: %d\n", gX_hat);
			printf("y_hat: %d\n", gY_hat);
			printf("theta_hat %d\n", gTheta_hat);
*/
			...
			// IMU calibration
			if(gyroCalib){//neccesary to complete this part only once
